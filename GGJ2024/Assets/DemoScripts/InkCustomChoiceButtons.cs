...
namespace InkEngine
{
    ...
    public class InkCustomChoiceButtons : MonoBehaviour
    { // This script lets you set up custom choice buttons based on what's in the gathered choices
        ...
        public bool m_hideCustomButtonsIfInvisible = true;
        ...
        public DialogueOptionsPresentedEvent m_onButtonPressedEvent;
        ...
        void Awake()
        {
            ...
            { // adds the ones added in the editor initially
        ...
        void Start()
        {
            ...
            foreach (CustomInkChoiceButton btn in m_customButtons)
            {
                ...
                if (m_hideCustomButtonsIfInvisible)
                {
        ...
        public void SetButtonActive(CustomInkChoiceButton button, bool active)
        {
            button.assignableButton.gameObject.SetActive(active);
        }
        ...
        void CheckChoices(List<(InkChoiceLine, Button)> args)
        {
            ...
            foreach ((InkChoiceLine, Button) button in args)
            {
                ...
                if (foundButton != null)
                {
                    ...
                    SetButtonActive(foundButton, true);
                };
            }
            if (buttonFound)
            {
        ...
        void ClickedButton(CustomInkChoiceButton targetButton)
        {
            ...
            if (m_hideCustomButtonsIfInvisible){
                SetButtonActive(targetButton, false);
            }
            targetButton.assignableButton.onClick.RemoveAllListeners(); // we also remove all listeners, just in case
            if (m_hideWriterIfFound)
            { // and we reopen the writer, if we closed it
                m_targetWriter.OpenCurrentDialogBox(false);
            }
            m_onButtonPressedEvent.Invoke(new List<(InkChoiceLine, Button)>{(null, targetButton.assignableButton)});
        }
